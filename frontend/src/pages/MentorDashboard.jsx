...
export const MentorDashboard = () => {
  ...
  const { data: myAvailability, refetch: refetchAvailability } = useFetch(
    user?.id ? `/api/availability/mentor/${user.id}` : null,
    {
      params: {
  ...
  const { data: myCalls } = useFetch(user?.id ? `/api/calls/mentor/${user.id}` : null);
  ...
  const handleAddAvailability = async (e) => {
    ...
    try {
      await api.post('/api/availability/mentor', {
        date: selectedDate,
        startTime,
        endTime,
        timezone,
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card">
              ...
              {myCalls && myCalls.length > 0 ? (
                <div className="space-y-3">
                  {myCalls.map((call) => (
                    <div key={call.id} className="p-4 bg-green-50 rounded-lg border border-green-200">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="font-semibold text-lg">{call.call_type_name}</p>
                          <p className="text-sm text-gray-600">User: {call.user_name}</p>
                        </div>
